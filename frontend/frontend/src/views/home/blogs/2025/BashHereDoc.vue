<template>
  <div class="bg-gray-50 min-h-screen">
    <div class="lg:hidden fixed top-4 right-4 z-50">
      <button @click="toggleMobileToc()" class="bg-blue-600 text-white p-3 rounded-full shadow-lg hover:bg-blue-700 transition-colors">
    ...
    <div id="mobileToc" class="lg:hidden fixed inset-0 z-40 bg-black bg-opacity-50" :class="{ 'hidden': isMobileTocHidden }" @click="closeMobileToc()">
      <div class="fixed top-0 right-0 h-full w-80 max-w-full bg-white shadow-xl overflow-y-auto" @click.stop="">
        <div class="p-4">
          ...
          <nav class="space-y-2" id="mobileTocNav">
            <a v-for="section in sections" :key="section.id" :href="'#' + section.id" class="block py-1 text-gray-600 hover:text-blue-600 transition-colors duration-200" @click="closeMobileToc()">
              {{ section.title }}
            </a>
          </nav>
        </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="mb-8">
        <h1 class="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 leading-tight mb-4">
          Mastering Bash HereDoc: A Complete Guide for System Administrators
        </h1>
        <div class="flex flex-wrap items-center gap-4 text-sm text-gray-600 mb-6">
          <span class="bg-blue-100 text-blue-800 px-3 py-1 rounded-full font-medium">Shell Scripting</span>
          <span class="bg-green-100 text-green-800 px-3 py-1 rounded-full font-medium">Linux</span>
          <span class="bg-purple-100 text-purple-800 px-3 py-1 rounded-full font-medium">DevOps</span>
          <span>December 2024 | 10 min read</span>
        </div>
      </div>
        ...
        <div class="hidden lg:block lg:w-80 flex-shrink-0">
          <nav class="sticky top-8 bg-white rounded-xl p-6 shadow-sm border">
            ...
            <div class="space-y-2" id="desktopTocNav">
              <a v-for="section in sections" :key="section.id" :href="'#' + section.id" class="block py-1 text-gray-600 hover:text-blue-600 transition-colors duration-200">
                {{ section.title }}
              </a>
            </div>
          </nav>
          ...
          <div class="bg-white rounded-xl p-6 sm:p-8 shadow-sm border">
            <div class="prose prose-lg max-w-none">
              <section id="introduction" class="mb-12">
                <h2 class="text-2xl sm:text-3xl font-bold mb-6">Why Bash HereDoc is a Game-Changer</h2>
                <p class="text-lg leading-relaxed mb-4">
                  As a system administrator working with complex cloud architectures, you've probably faced the challenge of executing multiple commands across different servers in a single shell script. Enter Bash HereDoc - a powerful feature that can transform how you handle multi-line input and streamline your automation workflows.
                </p>
                <p class="text-lg leading-relaxed mb-6">
                  Imagine having to access a public bastion host, then jump to private subnets, executing a series of commands in one seamless operation. HereDoc makes this not just possible, but elegant and maintainable. It's truly a life-saving method for DevOps professionals and system administrators.
                </p>
                  ...
                  <div class="flex">
                    ...
                    <div class="ml-3">
                      <p class="text-sm text-blue-700">
                        <strong>Pro Tip:</strong> HereDoc isn't just about multi-line strings - it's about creating maintainable, readable scripts that can handle complex deployment scenarios with ease.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <section id="what-is-heredoc" class="mb-12">
                ...
                <p class="text-lg leading-relaxed mb-4">
                  Bash HereDoc (Here Document) is a redirection method that allows you to pass multiple lines of input to a command. It's particularly useful when you need to:
                </p>
...
DELIMITER</code></pre>
                ...
                <p class="text-lg leading-relaxed">
                  The `DELIMITER` (commonly `EOF`) marks both the beginning and end of the input. Everything between these delimiters is treated as input to the preceding command.
                </p>
              </section>
              ...
              <section id="understanding-eof" class="mb-12">
                ...
                <p class="text-lg leading-relaxed mb-4">
                  EOF (End Of File) is the most common delimiter used in HereDoc. While you can use any word as a delimiter, EOF has become the standard convention because it clearly indicates the end of the input block.
                </p>
...
EOF</code></pre>
                ...
                <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 my-6">
                  <p class="text-sm text-yellow-700">
                    <strong>Important:</strong> The closing EOF must be on its own line with no leading or trailing characters, including spaces or tabs.
                  </p>
                </div>
              </section>

              <section id="quoted-unquoted-eof" class="mb-12">
                <h2 class="text-2xl sm:text-3xl font-bold mb-6">Quoted vs Unquoted EOF: Understanding the Difference</h2>
                <p class="text-lg leading-relaxed mb-6">
                  One of the most crucial concepts in HereDoc is understanding when to use quoted vs unquoted delimiters. This determines whether variable expansion occurs within your HereDoc block.
                </p>
...
EOF</code></pre>
                ...
                <p class="text-base mb-6 bg-gray-100 p-3 rounded">
                  <strong>Output:</strong> Variables and commands are expanded, showing actual values.
                </p>
...
EOF</code></pre>
                ...
                <p class="text-base mb-6 bg-gray-100 p-3 rounded">
                  <strong>Output:</strong> Literal text with $USERNAME, $(date), and $HOME printed as-is.
                </p>
                <div class="bg-green-50 border-l-4 border-green-400 p-4 my-6">
                  <p class="text-sm text-green-700">
                    <strong>Best Practice:</strong> Use quoted delimiters when creating configuration files or scripts where you want to preserve literal dollar signs and backticks.
                  </p>
                </div>
              </section>
              ...
              <section id="real-world-use-cases" class="mb-12">
                ...
                <h3 class="text-xl font-semibold mb-4">1. Bastion Host to Private Subnet Access</h3>
                <p class="text-lg leading-relaxed mb-4">
...
EOF</code></pre>
              ...
              <section id="nested-heredoc" class="mb-12">
                <h2 class="text-2xl sm:text-3xl font-bold mb-6">Advanced Technique: Nested HereDoc</h2>
                <p class="text-lg leading-relaxed mb-4">
                  Nested HereDoc allows you to embed one HereDoc inside another, enabling complex multi-level operations. This is particularly powerful for creating configuration files that include content from other files or commands.
                </p>
                <div class="bg-red-50 border-l-4 border-red-400 p-4 my-6">
                  <p class="text-sm text-red-700">
                    <strong>Critical Rule:</strong> Space is required between nested delimiters to separate them properly. Notice the space between `NESTED_EOF` and `EOF` in the examples below.
                  </p>
                </div>
...
EOF</code></pre>
              ...
              <section id="docker-best-practices" class="mb-12">
                <h2 class="text-2xl sm:text-3xl font-bold mb-6">Docker Volume Management Best Practices</h2>
                <p class="text-lg leading-relaxed mb-4">
                  An important note about Docker management: always clean up unused volumes to prevent disk space issues and maintain a clean environment.
                </p>
...
EOF</code></pre>
              ...
              <section id="performance-tips" class="mb-12">
                <h2 class="text-2xl sm:text-3xl font-bold mb-6">Performance Tips and Best Practices</h2>
                ...
                <div class="mt-6 bg-yellow-50 border border-yellow-200 rounded-lg p-6">
                  ...
                  <ul class="text-sm text-yellow-700 space-y-2">
                    <li>• Use descriptive delimiter names (e.g., `CONFIG_EOF` instead of `EOF`)</li>
              ...
              <section id="conclusion" class="mb-12">
                ...
                <p class="text-lg leading-relaxed mb-4">
                  Bash HereDoc is an indispensable tool for system administrators and DevOps engineers. It simplifies complex tasks by allowing you to embed multi-line strings and commands directly into your scripts. By mastering concepts like quoted vs. unquoted delimiters and nested HereDocs, you can create scripts that are not only more powerful but also more readable and maintainable. This guide has shown you the basics and several real-world examples, from managing Docker containers to performing multi-level SSH operations. Use these techniques to streamline your workflows, automate repetitive tasks, and focus on more strategic challenges in your infrastructure.
                </p>
              </section>
            </div>
          </div>
