<template>
  <div>
    ...
    <div class="max-w-screen-xl mx-auto p-4 sm:p-6 xl:p-8">
      <h1 class="text-3xl">
        {{ BLOGS_INFO['not_found_route_404']['title'] }}
      </h1>
      <p id="read-text">
        {{ BLOGS_INFO['not_found_route_404']['date'] }} |
        {{ BLOGS_INFO['not_found_route_404']['readTime'] }} read
      </p>
      <h2 class="text-lg my-3">
        It is quite common that we sometimes forget to handle undefined routes. If we do not handle
        undefined routes, it does not causes error but it results in empty blank page. Instead
        displaying empty blank page, it is good to display anything meaningful contents.
      </h2>
      <h2 class="text-lg my-3">For example: content not found or the route is not defined.</h2>
      <h1 class="text-xl font-bold">Technical Details</h1>
      <div class="my-3">
        <h1 class="text-xl font-bold">
          Step 1: Create new component and give any name. For my case, I have created NotFound.vue
        </h1>
      </div>
      ...
      <h1 class="my-3">
        '/:pathMatch(.*)*' => This custom param regexp will match everything and if the route is not
        found then will redirect to the NotFound.vue.
      </h1>

      <h1 class="text-xl font-bold">Bottom link provides in-depth information</h1>
      <h2 class="text-lg my-3">
        <a
          href="https://router.vuejs.org/guide/essentials/dynamic-matching.html#Catch-all-404-Not-found-Route"
          target="_blank"
          >Catch all / 404 Not found Route</a
        >
      </h2>
    </div>
    <Footer />
