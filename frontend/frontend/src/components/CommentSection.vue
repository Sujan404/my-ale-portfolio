<template>
    <div class="flex flex-col my-4 py-4 space-y-10 border-t-2">
        ...
        <!-- If the user is not authenticated-->
         ...
         <div v-else>
            <div v-if="this.commentSubmitSuccess">
                Your comment will show up here after is has been approved.
            </div>
            <form class="POST" @submit.prevent="submitComment">
                <textarea type="text"
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-300 focus:ring-opacity-50"
                row="5"
                v-model="commentContent"/>

                <button class="mt-4 w-full bg-teal-500 hover:bg-teal-700 focus:ring focus:ring-teal-100 text-white py-2 rounded-md text-lg tracking-wide">
                    submit Comment
                </button>
            </form>
         </div>
